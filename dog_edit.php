...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateQuery = "UPDATE tbl_231_dogs SET dogName = '$dogName', breed = '$breed', age = '$age', description = '$description'
    ...
    if ($result) {
        ...
        $errorMessage = 'Error updating dog. Please try again.';
    }
}
...
?>
    ...
    <div class="container">
        ...
        <form method="POST">
            <?php
            $dogData = $result->fetch_assoc();
            ?>
            <input type="hidden" name="dogId" value="<?php echo $dogData['id']; ?>">
            <div class="mb-3">
                <label for="dogName" class="form-label">Dog Name</label>
                <input type="text" class="form-control" id="dogName" name="dogName" value="<?php echo $dogData['dogName']; ?>" required>
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" id="breed" name="breed">
                    ...
                    foreach ($breeds as $breed) {
                        $selected = ($breed['name'] == $dogData['breed']) ? 'selected' : '';
                        echo '<option value="' . $breed['name'] . '" ' . $selected . '>' . $breed['name'] . '</option>';
                    }
                    ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="age" name="age" value="<?php echo $dogData['age']; ?>" required>
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Description</label>
                <input type="text" class="form-control" id="description" name="description" value="<?php echo $dogData['description']; ?>" required>
            </div>
            <div>
