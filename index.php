...
<body id="body-login">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($result->num_rows > 0) {
            ...
            $_SESSION['id'] = $user['id'];
            $_SESSION['dogNames'] = getDogNames($user['id']);

            header("Location: dog_select.php");
            exit();
        } else {
    ...
    function getDogNames($userID)
    {
        require 'connection.php';

        $sql = "SELECT dogName FROM tbl_231_dogs WHERE user_id = $userID";
        $result = $conn->query($sql);
        $dogNames = [];

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $dogNames[] = $row['dogName'];
            }
        }
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <ul class="list-unstyled">
                                <?php
                                if (isset($_SESSION['dogNames']) && is_array($_SESSION['dogNames'])) {
                                    foreach ($_SESSION['dogNames'] as $dogName) {
                                ?>
                                        <li class="media">
                                            <div class="media-body">
                                                <h5 class="mt-0 mb-1"><a href="home.php" class="text-decoration-none"><?php echo $dogName; ?></a></h5>
                                            </div>
                                        </li>
                                <?php
                                    }
